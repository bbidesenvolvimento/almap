...
if (!$arquivo) {
    $mensagens=$mensagens."Não acesse esse arquivo diretamente!<br />";
}
...
// Imagem foi enviada, então a move para o diretório desejado
    ...
    if (move_uploaded_file($arquivo['tmp_name'], $diretorio . $arquivo['name'])) {
        $mensagens=$mensagens. "Arquivo Enviado com sucesso!<br />";
        //die(var_dump($arquivo));
    } 
    else {
        $mensagens=$mensagens. "Erro ao enviar seu arquivo!<br />";
    }
}
	...
	if(mysql_affected_rows() == 1){ //verifica se foi afetada alguma linha, nesse caso inserida alguma linha
		$mensagens=$mensagens. "Cadastro feito com sucesso<br />";
		...
		$mensagens=$mensagens. "Erro, não possível inserir no banco de dados<br />";
	}
